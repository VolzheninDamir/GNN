...
class GNNPlusLayer(nn.Module):
    ...
    - Message Passing (TransformerConv с признаками рёбер)
    - Pre‑norm, Dropout, Residual
    - FFN (MLP) с Pre‑norm и Residual
    """
    def __init__(self, hidden_dim, dropout=0.1):
        super().__init__()
        # Используем TransformerConv (поддерживает edge_dim, не имеет add_self_loops)
        self.msg = TransformerConv(hidden_dim, hidden_dim, heads=3, concat=False,
                                   edge_dim=hidden_dim)
...
class GNNPlusHetero(nn.Module): 
    ...
    def __init__(self,
        ...
        # TransformerConv не имеет параметра add_self_loops, edge_dim не нужен
        self.well_conv = HeteroConv({
            ('cell', 'linked_to', 'well'): TransformerConv(hidden_dim, hidden_dim,
...
""" class GNNPlusHetero(nn.Module): 
    def __init__(self,
        ...
        # 2. Слои для графа ячеек
        self.cell_layers = nn.ModuleList([
            GNNPlusLayer(hidden_dim, dropout) for _ in range(num_layers)
        ])

        # 3. Слой для связи ячейка → скважина
        self.well_conv = HeteroConv({
            ('cell', 'linked_to', 'well'): TransformerConv(hidden_dim, hidden_dim,
                                                           heads=3, concat=False)
        }, aggr='mean')
        ...
        if use_lstm:
            ...
            self.pos_embedding = nn.Parameter(torch.randn(1, out_seq_len, hidden_dim) * 0.1)
            # LSTM (2 слоя, batch_first)
            self.lstm = nn.LSTM(
    ...
    def forward(self, data):
        # ... (код до well_updates без изменений) ...
        # Проецируем признаки
        x_dict = {
            'cell': self.cell_emb(data['cell'].x),
            'well': self.well_emb(data['well'].x)
        ...
        well_updates = self.well_conv(x_dict, data.edge_index_dict)
        h_well = well_updates['well']   # (N, hidden_dim)

        if self.use_lstm:
            # Повторяем h_well для каждого шага: (N, out_seq_len, hidden_dim)
            h_well_seq = h_well.unsqueeze(1).repeat(1, self.out_seq_len, 1)
            # Добавляем позиционное кодирование
            h_well_seq = h_well_seq + self.pos_embedding
            # LSTM
            lstm_out, _ = self.lstm(h_well_seq)   # (N, out_seq_len, hidden_dim)
            # Преобразуем в предсказания: (N, out_seq_len, num_phases)
            out = self.step_mlp(lstm_out)
            # Меняем порядок осей на (N, num_phases, out_seq_len)
            out = out.permute(0, 2, 1)
        else:
            out = self.well_mlp(h_well)
            out = out.view(-1, self.num_phases, self.out_seq_len)
