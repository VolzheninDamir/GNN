...
# ----------------------------------------------------------------------
# Модель с TransformerConv
# ----------------------------------------------------------------------
class GNNPlusLayerTransformer(nn.Module):
...
class GNNPlusHeteroTransformer(nn.Module):
    def __init__(self, cell_features, well_features, hidden_dim, out_seq_len=24,
        ...
        self.cell_layers = nn.ModuleList([
            GNNPlusLayerTransformer(hidden_dim, dropout, heads, edge_dim=hidden_dim)
...
def train_one_config(config, train_loader, val_loader, device, trial=None, verbose=True):
    ...
    model = GNNPlusHeteroTransformer(
...
def main():
    ...
    study = optuna.create_study(
        direction='minimize',
        study_name='transformerconv_hyperopt_no_prune',
    ...
    print("\n=== Результаты поиска для TransformerConv (без отсечения) ===")
    ...
    with open("best_params_transformerconv.yaml", "w") as f:
        yaml.dump(best_params, f, default_flow_style=False)
    print("\nЛучшие параметры сохранены в best_params_transformerconv.yaml")
