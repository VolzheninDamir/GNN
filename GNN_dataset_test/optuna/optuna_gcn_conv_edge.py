# optuna_gcnconv_edge_no_prune.py (исправленная версия)
import os
import torch
...
# ----------------------------------------------------------------------
# Кастомный GCNConv с поддержкой edge features (без self-loops)
# ----------------------------------------------------------------------
class GCNConvWithEdge(MessagePassing):
    def __init__(self, in_channels, out_channels, edge_dim=None, bias=True, **kwargs):
...
# ----------------------------------------------------------------------
# Блок GNN+ с GCNConvWithEdge
# ----------------------------------------------------------------------
class GNNPlusLayerGCN(nn.Module):
    def __init__(self, hidden_dim, dropout, edge_dim):
        super().__init__()
        self.msg = GCNConvWithEdge(hidden_dim, hidden_dim, edge_dim=edge_dim)
...
# ----------------------------------------------------------------------
# Гетерогенная модель с GCNConvWithEdge для ячеек и GATv2Conv для связи cell->well
# ----------------------------------------------------------------------
class GNNPlusHeteroGCN(nn.Module):
    def __init__(self, cell_features, well_features, hidden_dim, out_seq_len=24,
        ...
        self.cell_layers = nn.ModuleList([
            GNNPlusLayerGCN(hidden_dim, dropout, edge_dim=hidden_dim)
...
def train_one_config(config, train_loader, val_loader, device, trial=None, verbose=True):
    ...
    edge_dim = config['edge_dim']          # берём из config
    ...
    if verbose:
        print(f"\n[Training] Trial {trial.number if trial else 0}: "
              f"hidden={hidden_dim}, layers={num_layers}, dropout={dropout:.3f}, "
...
def objective(trial, train_loader, val_loader, device, edge_dim):
    ...
    config = {
        ...
        'heads': heads,
        'edge_dim': edge_dim,           # передаём размерность рёбер
        'epochs': 20,
        'patience': 1
    }
...
def main():
    ...
    edge_dim = len(config['preprocess']['edge_feature_list'])
    print(f"Размерность edge_attr в поиске: {edge_dim}")

    study = optuna.create_study(
        direction='minimize',
        study_name='gcnconv_edge_hyperopt_no_prune',
    ...
    print("\n=== Результаты поиска для GCNConv с поддержкой рёбер ===")
    ...
    with open("best_params_gcnconv_edge.yaml", "w") as f:
        yaml.dump(best_params, f, default_flow_style=False)
    print("\nЛучшие параметры сохранены в best_params_gcnconv_edge.yaml")
