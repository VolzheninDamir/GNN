...
# ----------------------------------------------------------------------
# Модель с GATv2Conv (полностью переопределена)
# ----------------------------------------------------------------------
class GNNPlusLayerGATv2(nn.Module):
    def __init__(self, hidden_dim, dropout, heads, edge_dim):
        super().__init__()
        self.msg = GATv2Conv(
...
class GNNPlusHeteroGATv2(nn.Module):
    def __init__(self, cell_features, well_features, hidden_dim, out_seq_len=24,
        ...
        self.cell_layers = nn.ModuleList([
            GNNPlusLayerGATv2(hidden_dim, dropout, heads, edge_dim=hidden_dim)
...
def train_one_config(config, train_loader, val_loader, device, trial=None, verbose=True):
    ...
    if verbose:
        print(f"\n[Training] Trial {trial.number if trial else 0}: "
              ...
              f"dropout={dropout:.3f}, lr={lr:.2e}, wd={weight_decay:.2e}")
...
# ----------------------------------------------------------------------
# Objective для Optuna (без отсечения)
# ----------------------------------------------------------------------
def objective(trial, train_loader, val_loader, device, edge_dim):
    heads = trial.suggest_categorical('heads', [4])          # фиксированное значение 4
    hidden_dim = trial.suggest_categorical('hidden_dim', [64])  # фиксированное 64
    num_layers = trial.suggest_int('num_layers', 3, 4)       # от 3 до 4
...
def main():
    ...
    study = optuna.create_study(
        direction='minimize',
        study_name='gatv2conv_hyperopt_no_prune',
    ...
    print("\n=== Результаты поиска для GATv2Conv (без отсечения) ===")
    ...
    with open("best_params_gatv2conv.yaml", "w") as f:
        yaml.dump(best_params, f, default_flow_style=False)
    print("\nЛучшие параметры сохранены в best_params_gatv2conv.yaml")
